 ...
 * @see PageFlipperDown
 */
public class PageFlipperUp extends Button {
    private MultiPageInventory multiPageInventory;
    private Label pageCounter;

    public PageFlipperUp(String componentName, ItemStack material, int location, boolean consistence) {
    ...
    public void activeOnTrigger(InventoryClickEvent event) {
        if (multiPageInventory.getCurrentPageIndex()==0){
            return;
        }
        multiPageInventory.felipPageUp();
